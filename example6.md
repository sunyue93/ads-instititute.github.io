...
envelope is $C^1$-smooth with the gradient $$\label{eqn:grad_form}
\nabla f_{\nu}(x)=\nu^{-1}(x-{\rm prox}_{\nu f}(x)).$$ Rearranging the
...
$$\|\nu^{-1}(x_t-x_{t+1})\|<\varepsilon,$$ are equivalent for the
...
composite problem class: $$\label{eqn:comp}
\min_{x}~ F(x):=g(x)+h(c(x)),$$ where
...
function: $$\label{eqn:add_comp}
        \min_{x}~ c(x)+g(x).$$ Such problems appear often in statistical
...
$$\min_{x\in {\mathbb R}^d}~ F(x)=\mathbb{E}_{\zeta} f(x,\zeta).$$ Here,
...
$$\mathbb{E}_{\zeta}[\|\nabla F_{2\rho}(x)\|^2]\leq \varepsilon$$ after
at most
...
$$\begin{aligned}
x_{t+1}=\argmin_x \{F(x;x_t)+\tfrac{\beta}{2}\|x-x_t\|^2\},\end{aligned}$$
where $F(x;y)$ is the local convex model
$$F(x;y):=g(x)+h\left(c(y)+\nabla c(y)(x-y)\right).$$ In other words,
...
prox-linear method $$\mathcal{G}(x):=\beta(x_{t+1}-x_t)$$ is a good
...
$$O\left(\frac{\beta}{\mu}\ln(1/\varepsilon)\right)$$ iterations.
...
$$O\left(\sqrt{\frac{\beta}{\mu}}\ln(1/\varepsilon)\right),$$ which is
...
$$O\left(m\sqrt{\frac{\beta}{\mu}}\ln(1/\varepsilon)\right),$$ since
...
$$O\left(\left(m+\frac{\beta}{\mu}\right)\ln(1/\varepsilon)\right),$$ in

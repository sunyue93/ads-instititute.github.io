...
where $$\Delta=\diag(w^{\new}-w)$$, $$(M_w)_S\in \mathbb{R}^{n \times r}$$ is the $$r$$ columns from $$S$$ of $$M_w$$ and $$(M_w)_{S,S},\Delta_{S,S} \in \mathbb{R}^{r \times r}$$ are the $$r$$ rows and columns from $$S$$ of $$M_w$$ and $$\Delta$$.
